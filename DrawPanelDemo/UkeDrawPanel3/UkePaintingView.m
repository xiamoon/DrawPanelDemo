...
@property (nonatomic, assign) BOOL isBlendedLayer;
...
- (void)drawWithMode:(UkeDrawingBrushType)drawingMode
    ...
    if (_currentDrawingMode == UkeDrawingBrushTypeBrush ||
        ...
        if (state & UkeDrawingStateEnd || forceEnd) {
            ...
            [self handleEraseActionRectangleEraser:NO lineWidth:width];
        }
        ...
        if (state & UkeDrawingStateEnd || forceEnd) {
            ...
            [self handleEraseActionRectangleEraser:YES lineWidth:width];
        }
    }
...
- (BOOL)checkInfinityValueForLayer:(UkeDrawingItemLayer *)layer {
...
- (void)endBlendWithContext:(CGContextRef)context
                  clearPath:(CGPathRef)clearPath
                drawingMode:(CGPathDrawingMode)drawingMode
                  lineWidth:(CGFloat)lineWidth
                targetLayer:(UkeDrawingItemLayer *)targetLayer
                  nextLayer:(nullable CALayer *)nextLayer {
    ...
    targetLayer.contents = (id)image.CGImage;
    UIGraphicsEndImageContext();
    
    if (nextLayer) {
        [self.layer insertSublayer:targetLayer below:nextLayer];
    } else {
...
- (void)handleEraseActionRectangleEraser:(BOOL)rectangleEraser lineWidth:(CGFloat)lineWidth {
    
    if (!_currentPath || !_currentPath.CGPath) return;

    [self hideLaserPenLayer];
    
    CGPathDrawingMode drawingMode = rectangleEraser ? kCGPathFill : kCGPathStroke;
    
    __block BOOL beginBlend = NO;
    __block CGContextRef context = NULL;
    __block UkeDrawingItemLayer *blendedLayer = nil;
    ...
    [sublayers enumerateObjectsUsingBlock:^(__kindof CALayer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([obj isKindOfClass:UkeDrawingItemLayer.class] == NO) {
            if (beginBlend == YES && idx == sublayers.count - 1) {
                [self endBlendWithContext:context clearPath:_currentPath.CGPath drawingMode:drawingMode lineWidth:lineWidth targetLayer:blendedLayer nextLayer:nil];
                beginBlend = NO;
            }
            return;
        }
        
        UkeDrawingItemLayer *sublayer = (UkeDrawingItemLayer *)obj;
        if ([self checkInfinityValueForLayer:sublayer] == NO) {
            if (beginBlend == YES && idx == sublayers.count - 1) {
                [self endBlendWithContext:context clearPath:_currentPath.CGPath drawingMode:drawingMode lineWidth:lineWidth targetLayer:blendedLayer nextLayer:nil];
                beginBlend = NO;
            }
        ...
        if (sublayer.isBlendedLayer == NO && sublayer.type <= UkeDrawingBrushTypePressurePen) {
            if (beginBlend == NO) {
                blendedLayer = [[UkeDrawingItemLayer alloc] init];
                blendedLayer.frame = self.bounds;
                blendedLayer.isBlendedLayer = YES;
            ...
            if (beginBlend == YES) {
                [self endBlendWithContext:context clearPath:_currentPath.CGPath drawingMode:drawingMode lineWidth:lineWidth targetLayer:blendedLayer nextLayer:sublayer];
                beginBlend = NO;
            }
            ...
            [eraseLayer appendErasePath:_currentPath.CGPath drawingMode:drawingMode width:lineWidth];
            obj.mask = eraseLayer;
        }
    }];
}
