...
- (void)drawWithPoints:(NSArray<NSArray *> *)points {
    ...
    [self.pointParser parseWithPoints:points completion:^(UkeDrawingPointsParser * _Nonnull parser) {
        ...
        if ([parser brushType] == UkeDrawingBrushTypeEdit_regular) { // 图形编辑
            
            [weakSelf.paintingView editWithEditInfo:parser.editInfo identifier:parser.identifier];
            
        } else if ([parser brushType] == UkeDrawingBrushTypeEdit_delete) { // 图形删除
            
            [weakSelf.paintingView deleteWithIdentifiers:parser.delete_ids];
            
        } else if ([parser brushType] == UkeDrawingBrushTypeEdit_deletePage) { // 清除整页
            
            [weakSelf.paintingView deleteCurrentPage];
            
        } else if ([parser brushType] == UkeDrawingBrushTypeText ||
            [parser brushType] == UkeDrawingBrushTypeText_edit) {  // 画文字
            
            [weakSelf.paintingView drawTextWithText:[parser text]
                                         startPoint:[parser startPoint]
                                           fontSize:[parser lineWidth]
                                              color:[parser color] identifier:parser.identifier targetLayer:nil transform:CGPointMake(1.0, 1.0)];
            
        } else { // 画图形
            [weakSelf.paintingView drawWithMode:[parser brushType] drawingState:[parser drawingState] startPoint:[parser startPoint] otherPoints:[parser realDrawPoints] width:[parser lineWidth] color:[parser color] isFillPath:[parser isFillPath] isNormalShape:[parser isNormalShape] forceEnd:[parser forceEndPreviousPath] identifier:parser.identifier targetLayer:nil showVertex:parser.showVertex];
        }
    }];
}
